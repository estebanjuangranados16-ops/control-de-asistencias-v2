...
const Employees = () => {
  ...
  // WebSocket listeners para empleados
  useEffect(() => {
    if (!socket) return

    socket.on('employee_added', (data) => {
      console.log('👤 Empleado agregado en tiempo real:', data)
      fetchEmployees()
      success(`Empleado ${data.name} agregado ${data.synced ? 'y sincronizado' : 'pero no sincronizado'} con el dispositivo`)
    })

    socket.on('employee_updated', (data) => {
      console.log('👤 Empleado actualizado:', data)
      fetchEmployees()
      success(`Empleado ${data.name} actualizado`)
    })
