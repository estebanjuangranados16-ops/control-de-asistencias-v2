...
const Dashboard = () => {
  ...
  useEffect(() => {
    fetchDashboardData()
    // Polling reducido cuando hay WebSocket
  ...
  useEffect(() => {
    ...
    socket.on('attendance_record', (data) => {
      console.log('📋 EVENTO TIEMPO REAL:', data)
    ...
    socket.on('connection_lost', (data) => {
      console.log('⚠️ Conexión perdida:', data.message)
      setDashboardData(prev => ({ ...prev, connected: false }))
    })

    socket.on('connection_restored', (data) => {
      console.log('✅ Conexión restaurada:', data.message)
      setDashboardData(prev => ({ ...prev, connected: true }))
      fetchDashboardData()
    })
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="text-center mb-12">
          ...
          <div className="flex items-center justify-center mt-4 space-x-4">
            ...
            <div className={`flex items-center space-x-2 px-4 py-2 rounded-full ${
              socketConnected ? 'bg-purple-100 text-purple-800' : 'bg-orange-100 text-orange-800'
            }`}>
              {socketConnected ? <Wifi className="w-4 h-4" /> : <WifiOff className="w-4 h-4" />}
              <span className="text-sm font-medium">
                {socketConnected ? 'Tiempo Real Activo' : 'Tiempo Real Inactivo'}
              </span>
            </div>
            {lastUpdate && (
              <div className="flex items-center space-x-2 px-4 py-2 rounded-full bg-yellow-100 text-yellow-800">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-white/20">
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              <div className="text-xs text-gray-400 mb-2 flex items-center justify-between">
                <span>Registros: {dashboardData.recent_records?.length || 0}</span>
                {socketConnected && (
              ...
              {!dashboardData.recent_records || dashboardData.recent_records.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-xs text-gray-400">Conecta el dispositivo para ver actividad</p>
                </div>
              ) : (
