...
class UnifiedAttendanceSystem:
    ...
    def record_attendance(self, employee_id, timestamp, reader_no=1, verify_method="huella"):
        ...
        # Emitir evento en tiempo real
        print(f"EMITIENDO WEBSOCKET: {employee[0]} - {event_type}")
    ...
    def _process_event(self, event):
        ...
        if 'AccessControllerEvent' in event:
            ...
            if sub_type == 38:  # Acceso autorizado
                ...
                if employee_id:
                    print(f"EVENTO HUELLA: Empleado {employee_id}")
                    success = self.record_attendance(employee_id, timestamp, reader_no, verify_method)
                    if success:
                        print(f"EVENTO PROCESADO Y ENVIADO")
                    else:
                        print(f"ERROR: Empleado {employee_id} no existe en BD")
...
def api_update_employee(employee_id):
    ...
    try:
        cursor.execute('''
            ...
            WHERE employee_id = ?
        ''', (data['name'], data.get('department', 'General'), 
              data.get('phone', ''), data.get('email', ''), employee_id))
...
def api_delete_employee(employee_id):
    ...
    try:
        ...
        return jsonify({'success': True, 'message': f'Empleado {employee[0]} eliminado correctamente'})
...
def api_toggle_employee(employee_id):
    ...
    try:
        ...
        name, current_active = employee
        new_active = not current_active
        
        # Actualizar estado
        cursor.execute('UPDATE employees SET active = ? WHERE employee_id = ?', (new_active, employee_id))
        conn.commit()
        conn.close()
        ...
        return jsonify({'success': True, 'message': f'Empleado {name} {status} correctamente'})
